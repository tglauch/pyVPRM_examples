...
hvs = np.unique(
    [
        lat_lon_to_modis(
            out_grid["lat_b"].values.flatten()[i], out_grid["lon_b"].values.flatten()[i]
        )
...
# days of interest
days = [datetime(this_year, 1, 1) + timedelta(days=i) for i in np.arange(365.0)]
    ...
    for c0, fpath in enumerate(file_collections):
        ...
        if c0 == 0:
            trans = Transformer.from_crs(
                "+proj=longlat +datum=WGS84", handler.sat_img.rio.crs
            )
...
for key in wrf_op.keys():
    ofile = os.path.join(
        ...
        file_base
        + filename_dict[key]
        + "_{}_part_{}_{}.nc".format(this_year, args.chunk_x, args.chunk_y),
    )
